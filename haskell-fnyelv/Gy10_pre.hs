...
decimalFractional = do
  ...
  return $ case frac of
      ...
      Just fracDigs  -> fromIntegral int + digitsToDecimalFraction fracDigs
...
pIntLit :: Parser Exp
pIntLit = Lit <$> decimalNumber <* ws

pZjel :: Parser Exp
pZjel = char' '(' *> pPlus <* char' ')'

pAtom :: Parser Exp
pAtom = pIntLit <|> pZjel

-- mit olvasok és mivel van elválasztva
