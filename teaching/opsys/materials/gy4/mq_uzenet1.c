...
int main(int argc, char *argv[]) {
    ...
    if (child > 0) {
        ...
        //
        printf("[child] Buffer length: %d\n", strlen(buffer));
        printf("[child] mq_receive : %s, olvasott bajtok hossza: %d \nerrno: %s\n", buffer, db, strerror(errno));
        db = mq_receive(mq1, buffer, 50, 0);
        printf("[child] kovetkezo message: %s hossza: %d\n", buffer, db);
        mq_close(mq1);
        //mq_send(mq1,"Gyerek befejezte!", 20,6);
        //msgsnd(uzenetsor,&valasz,sizeof(struct message),0);
        return 0;
